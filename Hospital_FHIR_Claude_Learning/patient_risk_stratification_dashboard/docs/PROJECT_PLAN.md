...
## Build status (as of pause)
- ✅ `.venv` + `requirements.txt`; ✅ `config.py`; ✅ `ingest.py` (real 101,766-row download verified); ✅ `clean.py` (verified: 69,990 rows, 8.98% positive).
- ⏸ Paused before `features.py`. Remaining: features → model → risk_scoring → explain → export → notebook → tests → README → verification.
